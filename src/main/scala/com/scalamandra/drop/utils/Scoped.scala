...
given [T <: Batch]: Scoped[T] = new Scoped[T] {

  override inline def begin(self: T): Unit = self.begin()

  override inline def end(self: T): Unit = self.end()
...
given [T]: Scoped[DelayedRemovalArray[T]] = new Scoped[DelayedRemovalArray[T]] {
  ...
  override inline def begin(self: DelayedRemovalArray[T]): Unit = self.begin()

  override inline def end(self: DelayedRemovalArray[T]): Unit = self.end()
